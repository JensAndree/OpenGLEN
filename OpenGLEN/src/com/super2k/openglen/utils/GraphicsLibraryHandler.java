...
public abstract class GraphicsLibraryHandler {
     ...
     */
    public abstract void setupGL(RenderSetting setting);
                ...
                if (failed) {
                    for (int i = 0; i < bufferTarget.length; i++) {
                        if (bufferTarget[i] != 0) {
                            ...
                            bufferData(bufferTarget[i], 0, null, ConstantValues.STATIC_DRAW);
                        }
                    }
                    deleteBuffers(2, bufferNames, 0);
                }
            }
        } //end while
    }
    ...
    public void deleteVBOBuffer(int[] names, int count, int index, int target) {
        ...
        for (int i = index; i < index + count; i++) {
            if (names[i] != 0) {
                ...
                //                bufferData(target, 0, null, ConstantValues.STATIC_DRAW);
    ...
    /**
     * Delete all gl buffers and textures in the specified list and removes the
     * objects from the list.
     * Use this method when you want to discard all the objects, bitmaps and textures in a list,
     * for instance when an application is exiting.
     * @param list List with GLBLitObject to delete the buffers and textures for.
     * @throws IllegalArgumentException If list is null.
     */
    public static void releaseGLBuffers(Vector<GLBlitObject> list, Renderer renderer) {
        if (list==null) {
        ...
        for (int i = 0; i<count; i++) {
            ...
            if (blit.material != null && blit.material.texture != null) {
                textures = blit.material.texture;
                for (int loop = 0; loop < textures.length; loop++) {
                    names[0] = textures[loop].getTextureName();
                    renderer.getTextureHandler().deleteTextures(1, names, 0);
                }
            }
            renderer.getGraphicsUtilities().deleteVBOBuffers(blit);
            blit.destroy();
        }
